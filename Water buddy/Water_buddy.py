...
def speak(text, mood="neutral", lang="en"):
    ...
    # Don't prepend language name, just use the raw text
    text_js = str(text).replace('"','\\"').replace("\n","\\n")
    pitch = 1.0
    rate = 1.0
    volume = 1.0
    if mood == "happy":
    ...
    <script>
      try {{
        ...
        utter.rate = {rate};
        utter.volume = {volume};
        var matchLang = "{voice_lang}";
        var voices = speechSynthesis.getVoices();
        if (voices && voices.length) {{
          var exact = voices.find(v=>v.lang===matchLang);
          if (exact) utter.voice = exact;
          else {{
            var close = voices.find(v=>v.lang && v.lang.startsWith(matchLang.split('-')[0]));
            if (close) utter.voice = close;
          }}
        }}
